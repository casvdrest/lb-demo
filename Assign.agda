...
-- Soundness for expressions follows from the lemmas 'E-preservation' and 'E-progress'.
--
-- Progress tells us that well-typed expressions are either a literal, or they can be reduced.
-- Preservation tells us that the reduced expression is also well-typed (with the same type).
--
...
-- Assignment
preservation ([P-cons] ([S-assign] x ns) d) hwt (─→-assign₁ st)
  with E-preservation x hwt st
... | r = [P-cons] ([S-assign] r ns) d
preservation ([P-cons] ([S-assign] [E-nlit] ns) d) hwt (─→-assign isNum)
  = d
preservation ([P-cons] ([S-assign] [E-blit] ns) d) hwt (─→-assign isBool)
  = d
...
progress [P-empty]
  = right isEmpty
progress ([P-cons] ([S-assign] d _) _)
  with E-progress d
... | left (_ , st)
  = left (_ , (─→-assign₁ st))
... | right l
